...
# 验证匹配密钥对（ECDSA pkey P-256）
openssl ec -in privateKey.key -pubout -outform pem -out private_pubkey.pem
openssl x509 -in certificate.crt -pubkey -noout -out cert_pubkey.pem > cert_pubkey.pem
diff cert_pubkey.pem private_pubkey.pem
