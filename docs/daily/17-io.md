...
```bash
# 启用Smart检查
smartctl -s on /dev/sda # SSD 256G
smartctl -s on /dev/sdb # HDD 1T
smartctl -s on /dev/sdc # HDD 1T
smartctl -s on /dev/sdd # HDD 1T

# 快速判断硬盘是否健康
smartctl -H /dev/sdd

smartctl -x /dev/sdd > sdd-x.txt

# 查看详细SMART属性
smartctl -A /dev/sdd > sdd-A.txt
grep -E "Reallocated_Sector_Ct|Current_Pending_Sector|Offline_Uncorrectable|Power_On_Hours|Temperature_Celsius" sdd-A.txt
...
# 坏道扫描
badblocks -b 4096 -v /dev/sdd > sdd-badblocks.txt

# 顺序写测试（裸设备）
fio --name=seq_write_test --rw=write --bs=1M --size=10G \
  --runtime=30 --ioengine=libaio --direct=1 --group_reporting --filename=/dev/sdd

# 顺序读测试
fio --name=seq_read_test --rw=read --bs=1M --size=10G \
  --runtime=30 --ioengine=libaio --direct=1 --group_reporting --filename=/dev/sdd

# 随机写测试
fio --name=rand_write_test --rw=randwrite --bs=4k --size=10G \
  --runtime=30 --ioengine=libaio --direct=1 --group_reporting --filename=/dev/sdd

# 随机读测试
fio --name=rand_read_test --rw=randread --bs=4k --size=10G \
  --runtime=30 --ioengine=libaio --direct=1 --group_reporting --filename=/dev/sdd
```
